...
with open("point_to_projection_onto_line_tait_bryan_wc_jacobian.h",'w') as f_cpp: 
    ...
    f_cpp.write("}")
    f_cpp.write("\n")
    f_cpp.write("inline void point_to_projection_onto_line_tait_bryan_wc_jacobian(Eigen::Matrix<double, 3, 6> &j, double tx, double ty, double tz, double om, double fi, double ka, double x_src_l, double y_src_l, double z_src_l, double x_trg_g, double y_trg_g, double z_trg_g, double x_trg_ln, double y_trg_ln, double z_trg_ln)\n")
    f_cpp.write("{")
    for i in range (3):
        for j in range (6):
